...
void process_stream_stdin(char *fpntr); // process_stream FOR STDIN
...
int main(int argc, char *argv[])
{
  ...
  switch (c){
    ...
    case 's':
      ...
      break;
    default:
      break; // JUST BREAK IF NO OPTIONS, WE HAVE DEFAULT SETTINGS SET ALREADY
}
  ...
  {
    char ch[100];
    while(scanf(" %[^\n]", ch) == 1){ // SCANF UNTIL NEWLINE THEN REPEAT
      process_stream_stdin(ch); // SEND CH ARRAY TO process_stream_stdin
    }
  }
...
{
  int linenum = 1; // count the number of lines
  ...
  while((line = fgetline(fpntr)) != NULL) // LOOP TILL fgetline RETURNS NULL
  {
      if(s == NONEMPTY_LINES) // -bt
      {
        ...
        {
          printf("     %i%s%s", linenum++, string, line); // FORMAT FOR NONEMPTY
        }
      }
      else if(s == ALL_LINES) // -ba
      {
        printf("     %i%s%s", linenum++, string, line); // FORMAT FOR ALL
      }
      else if(s == NO_LINES) // -bn
      {
        printf("     %s%s", string, line); // FORMAT FOR NONE(NO LINENUM)
      }
        printf("\n"); // ADD A NEWLINE
    }
}

  void process_stream_stdin(char *fpntr)
  {
        if(s == NONEMPTY_LINES) // -bt
        {
          if(fpntr[0] != '\0') // DIFFERENCE BETWEEN ALL(ONLY PRINT ON NONEMPTY)
          {
            printf("     %i%s%s", inputlinenum++, string, fpntr); // FORMAT FOR NONEMPTY
          }
        }
        else if(s == ALL_LINES) // -ba
        {
          printf("     %i%s%s", inputlinenum++, string, fpntr); // FORMAT FOR ALL
        }
        else if(s == NO_LINES) // -bn
        {
            printf("     %s%s", string, fpntr); // FORMAT FOR NONE(NO LINENUM)
        }
        printf("\n"); // PRINT NEWLINE
    }
